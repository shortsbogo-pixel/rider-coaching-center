...
function getRequestedWeekKey(raw: unknown) {
  ...
  return weekKey && weekKey !== "all" ? weekKey : undefined;
}
...
router.get("/", async (req, res, next) => {
  try {
    ...
    res.json(await getRiderMetrics({ weekKey: getRequestedWeekKey(req.query.weekKey) }));
  } catch (error) {
    next(error);
  }
});
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const rider = (await getRiderMetrics({ weekKey: getRequestedWeekKey(req.query.weekKey) })).find((item) => item.riderId === req.params.id);
    if (!rider) {
